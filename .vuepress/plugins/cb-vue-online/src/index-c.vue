  ...
  >
    ...
    >
      <template v-slot:editor>
        <!-- 编辑器区域 -->
        <vue-run-sfc-editor class="panel" @change="compile"></vue-run-sfc-editor>
      </template>
...
import VueRunSfcEditor from "./components/vue-run-sfc-editor";
...
export default {
  ...
  methods: {
    ...
    async compile(code) {
      ...
      const imports = [];
      const { template, script, styles, customBlocks } = compiler.parseComponent(code);

      console.log(template, script, styles, customBlocks);
      let config;
